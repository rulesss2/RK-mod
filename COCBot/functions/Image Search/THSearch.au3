...
Func THSearch($bReTest = False)
	...
	If $debugsetlog = 1 Then SetLog("TH search Start", $COLOR_DEBUG) ;Debug
	...
	For $i = 0 To $NumTownHall - 1
		...
		If isInsideDiamond($pixel) Then
			...
			If $debugsetlog = 1 Then SetLog("TownHall: [" & $aTownHallLocal[0] & "," & $aTownHallLocal[1] & "], Level: " & $aTownHallLocal[2], $COLOR_DEBUG) ;Debug
			Return $THText[($aTownHallLocal[2] < 6 ? 0 : $aTownHallLocal[2] - 6)]
		Else
			If $debugsetlog = 1 Then SetLog("TownHall: [" & $pixel[0] & "," & $pixel[1] & "], Level: " & $level, $COLOR_DEBUG) ;Debug
			If $debugsetlog = 1 Then SetLog("Found TownHall with Invalid Location?", $COLOR_DEBUG) ;Debug
	...
	If $aTownHallLocal[0] = -1 Or $aTownHallLocal[1] = -1 Then
		If $debugImageSave = 1 Then DebugImageSave("checkTownhallADV2_NoTHFound_CSH_", True)
		If $debugsetlog = 1 Then SetLog(" == TownHall Not Found ==", $COLOR_DEBUG) ;Debug
		Return "-"
	EndIf
