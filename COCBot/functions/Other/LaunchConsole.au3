...
Func LaunchConsole($cmd, $param, ByRef $process_killed, $timeout = 10000, $bUseSemaphore = False)
	...
	If $debugSetlog = 1 Then Setlog("Func LaunchConsole: command launched", $COLOR_DEBUG) ;Debug
	If $pid = 0 Then
	...
	While True
		...
		;If $debugSetlog = 1 Then Setlog("Func LaunchConsole: StdoutRead...", $COLOR_DEBUG) ;Debug
		$data &= StdoutRead($pid)
		If @error Then ExitLoop
		;$data &= StderrRead($pid)
		If ($timeout > 0 And TimerDiff($hTimer) > $timeout) Then ExitLoop
		;If $debugSetlog = 1 Then Setlog("Func LaunchConsole: StdoutRead loop", $COLOR_DEBUG) ;Debug
	WEnd
	...
	If ProcessExists($pid) Then
		If ProcessClose($pid) = 1 Then
			If $debugSetlog = 1 Then SetLog("Process killed: " & $cmd, $COLOR_DEBUG) ;Debug
			$process_killed = True
		EndIf
	EndIf
	StdioClose($pid)
