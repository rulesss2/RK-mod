...
Func ClickRemove($FeatureName = "Remove")
	If _CheckPixel($aIsShieldInfo, $bCapturePixel) Then ; check for open shield info window
		...
		While 1 ; wait window with remove button
			...
			If $debugSetlog = 1 Then Setlog($FeatureName & " btn chk-#1: " & _GetPixelColor(475, 255, True) & ", #2: " & _GetPixelColor(475 + 109, 255, True) & ", #3: " & _GetPixelColor(475 + 65, 255 + 10, True) & ", #4: " & _GetPixelColor(475 + 55, 255 + 20, True), $COLOR_DEBUG) ;Debug
			If IsArray($ButtonPixel) Then
				If $debugSetlog = 1 Then
					Setlog("ButtonPixelLocation = " & $ButtonPixel[0] & ", " & $ButtonPixel[1], $COLOR_DEBUG) ;Debug
					Setlog("Pixel color found #1: " & _GetPixelColor($ButtonPixel[0], $ButtonPixel[1], True) & ", #2: " & _GetPixelColor($ButtonPixel[0] + 109, $ButtonPixel[1], True) & ", #3: " & _GetPixelColor($ButtonPixel[0] + 65, $ButtonPixel[1] + 10, True) & ", #4: " & _GetPixelColor($ButtonPixel[0] + 55, $ButtonPixel[1] + 20, True), $COLOR_DEBUG) ;Debug
				EndIf
		...
		Return True
	Else
		If $debugSetlog = 1 Then Setlog($FeatureName & " remove button found", $COLOR_DEBUG) ;Debug
		Return False
	EndIf
EndFunc   ;==>ClickRemove
