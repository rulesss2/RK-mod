...
Func saveConfig() ;Saves the controls settings to the config
	...
	Else
		$ichkPic = 0
	EndIf
	;IniWrite($config, "deletefiles", "chkPic", GUICtrlRead($ichkPic))

	;If GUICtrlRead($ichkPic) = $GUI_CHECKED Then
	;	IniWriteS($config, "deletefiles", "chkPic", 1)
	;Else
	;	IniWriteS($config, "deletefiles", "chkPic", 0)
	;EndIf

	;If GUICtrlRead($chkPic) = $GUI_UNCHECKED Then
	;	IniWriteS($config, "deletefiles", "chkPic", 0)
	;Else
	;	IniWriteS($config, "deletefiles", "chkPic", 1)
	;EndIf
	...
	;IniWrite($config, "deletefiles", "chkPic", "")
	;IniWriteS($config, "deletefiles", "chkPic", $chkPic)
	...
	If GUICtrlRead($chkOutSideCollectors) = $GUI_CHECKED Then
		$ichkOutSideCollectors = 1
	Else
		$ichkOutSideCollectors = 0
	EndIf
	$itxtSensCollectors = Int(GUICtrlRead($txtSensCollectors))
	$stxtPercentCollectors = String(GUICtrlRead($txtPercentCollectors))
	...
	Else
		$ichkOutSideMines = 0
	EndIf
	$itxtSensMines = Int(GUICtrlRead($txtSensMines))
	$stxtPercentMines = String(GUICtrlRead($txtPercentMines))
	...
	Else
		$ichkOutSideDrills = 0
	EndIf
	$itxtSensDrills = Int(GUICtrlRead($txtSensDrills))
	$stxtPercentDrills = String(GUICtrlRead($txtPercentDrills))
	; =========================== END OutSide Check ===========================
...
;==============================================================
	...
	IniWriteS($config, "collectors", "ichkOutSideCollectors", $ichkOutSideCollectors)
	IniWriteS($config, "collectors", "itxtSensCollectors", $itxtSensCollectors)
	IniWriteS($config, "collectors", "stxtPercentCollectors", $stxtPercentCollectors)

	IniWriteS($config, "collectors", "ichkOutSideMines", $ichkOutSideMines)
	IniWriteS($config, "collectors", "itxtSensMines", $itxtSensMines)
	IniWriteS($config, "collectors", "stxtPercentMines", $stxtPercentMines)

	IniWriteS($config, "collectors", "ichkOutSideDrills", $ichkOutSideDrills)
	IniWriteS($config, "collectors", "itxtSensDrills", $itxtSensDrills)
	IniWriteS($config, "collectors", "stxtPercentDrills", $stxtPercentDrills)
