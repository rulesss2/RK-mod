	...
	If UBound($pixel) = 2 Then
		...
		For $i = 0 To UBound($vector) - 1
			...
			If UBound($pixel2) = 2 Then
				...
				EndIf
			Else
				If $DebugSetLog = 1 Then Setlog("MilkFarmObjectivesSTR_INSERT Discard error #3", $COLOR_DEBUG) ;Debug
			EndIf
		Next
		...
		EndIf
	Else
		If $DebugSetLog = 1 Then Setlog("MilkFarmObjectivesSTR_INSERT Discard error #1 " & $pixel & " " & UBound($pixel), $COLOR_DEBUG) ;Debug
	EndIf
