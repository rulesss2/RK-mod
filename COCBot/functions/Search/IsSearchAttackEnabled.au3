...
Func IsSearchAttackEnabled()
	...
	If $debugsetlog = 1 Then Setlog("Begin IsSearchAttackScheduled:", $COLOR_DEBUG) ;Debug
	...
	If $debugsetlog = 1 Then Setlog("$bCloseGame:" & $bCloseGame, $COLOR_DEBUG) ;Debug
			...
			If $bCloseGame Then
				...
				If $iPlannedAttackWeekDays[@WDAY - 1] = 0 Then
					...
					For $i = @WDAY To 6
						...
						If $debugsetlog = 1 Then Setlog("Subtotal wait time= " & $iWaitTime & " Seconds", $COLOR_DEBUG) ;Debug
					Next
				EndIf
				If $iWaitTime = 0 Then ; if days are not set then compute wait time from hours
					...
					EndIf
				EndIf
				If $debugsetlog = 1 Then Setlog("Stop attack wait time= " & $iWaitTime & " Seconds", $COLOR_DEBUG) ;Debug
...
Func _getTimeRemainTimeToday()
	...
	If @error Then
		...
		Return
	EndIf
	If $debugsetlog = 1 Then Setlog("getTimeRemainToday= " & $iTimeRemain & " Seconds", $COLOR_DEBUG) ;Debug
	Return $iTimeRemain
...
Func _IsTimeAfter($sCompareTime, $sCurrentTime = _NowCalc())
	...
	If @error Then
		...
		Return
	EndIf
	If $debugsetlog = 1 Then Setlog("IsTimeAfter: " & $bResult, $COLOR_DEBUG) ;Debug
	Return $bResult
...
Func _IsTimeBefore($sCompareTime, $sCurrentTime = _NowCalc())
	...
	If @error Then
		...
		Return
	EndIf
	If $debugsetlog = 1 Then Setlog("IsTimeBefore: " & $bResult, $COLOR_DEBUG) ;Debug
	Return $bResult
...
Func _IsTimeInRange($sStartTime, $sEndTime)
	...
	If $debugsetlog = 1 Then Setlog("IsTimeInRange: " & $bResult, $COLOR_DEBUG) ;Debug
...
Func _getDailyRandomStartEnd($iDuration = 4)
	...
	If $iNowDay <> @YDAY Then ; if 1 day or more has passed since last time, compute new random start/stop times
		...
		If $debugsetlog = 1 Then Setlog("StartHour: " & $iStartHour & "EndHour: " & $iEndHour, $COLOR_DEBUG) ;Debug
		...
		EndIf
	EndIf
	If $debugsetlog = 1 Then Setlog("NoAttackStart: " & $aNoAttackTimes[0] & "NoAttackEnd: " & $aNoAttackTimes[1], $COLOR_DEBUG) ;Debug
...
Func _getDailyRandom()
	...
	If $iNowDay <> @YDAY Then ; if 1 day or more has passed since last time, update daily random value
		...
		If $debugsetlog = 1 Then Setlog("New day = new random value!", $COLOR_DEBUG) ;Debug
	EndIf
	If $debugsetlog = 1 Then Setlog("DailyRandomValue=" & StringFormat("%0.5f", $iDailyRandomValue), $COLOR_DEBUG) ;Debug
...
Func IsPlannedTimeNow()
	...
	If $iPlannedAttackWeekDays[@WDAY - 1] = 1 Then
		...
		If $iPlannedattackHours[$hourloot] = 1 Then
			If $debugsetlog = 1 Then SetLog("Attack plan enabled for now..", $COLOR_DEBUG) ;Debug
			Return True
		Else
...
Func _OverAttackLimit()
	...
	EndIf
	If $debugsetlog = 1 Then Setlog("AttackCountToday: " & $iAttackCountToday & ", AttackedCount: " & $iAttackedCount & "TotalAttackCount: " & $iTotalAttackCount, $COLOR_DEBUG) ;Debug
	...
	If $debugsetlog = 1 Then Setlog("RandomAttackCountToday: " & $iRandomAttackCountToday, $COLOR_DEBUG) ;Debug
