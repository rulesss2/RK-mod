...
Func isBarrack()
	...
	For $i = 1 To 4
		If _ColorCheck(_GetPixelColor($btnpos[$i][0], $btnpos[$i][1], True), Hex(0xE8E8E0, 6), 10) Then
			If $debugsetlogTrain = 1 Then SetLog("Barrack" & $i & " selected", $COLOR_DEBUG) ;Debug
			Return True ;exit when  found
		EndIf
	Next

	If $debugsetlogTrain = 1 Then SetLog("This is not a Barrack", $COLOR_DEBUG) ;Debug
	Return False
EndFunc   ;==>isBarrack
...
Func isDarkBarrack()
	...
	For $i = 5 To 6
		If _ColorCheck(_GetPixelColor($btnpos[$i][0], $btnpos[$i][1], True), Hex(0xE8E8E0, 6), 10) Then
			If $debugsetlogTrain = 1 Then SetLog("Dark Barrack" & $i - 4 & " selected", $COLOR_DEBUG) ;Debug
			Return True ;exit when found
		EndIf
	Next
	If $debugsetlogTrain = 1 Then SetLog("This is not a Dark Barrack", $COLOR_DEBUG) ;Debug
	Return False
...
Func isSpellFactory()
	...
	If _ColorCheck(_GetPixelColor($btnpos[7][0], $btnpos[7][1], True), Hex(0xE8E8E0, 6), 10) Then
		If $debugsetlogTrain = 1 Then SetLog("Spell FactoryDark  selected", $COLOR_DEBUG) ;Debug
...
Func isDarkSpellFactory()
	If _ColorCheck(_GetPixelColor($btnpos[8][0], $btnpos[8][1], True), Hex(0xE8E8E0, 6), 10) Then
		If $debugsetlogTrain = 1 Then SetLog("Dark Spell Factory  selected", $COLOR_DEBUG) ;Debug
