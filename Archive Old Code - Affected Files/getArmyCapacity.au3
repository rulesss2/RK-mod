...
Func getArmyCapacity($bOpenArmyWindow = False, $bCloseArmyWindow = False)
	...
	If $debugsetlogTrain = 1 Or $debugSetlog = 1 Then SETLOG("Begin getArmyCapacity:", $COLOR_DEBUG) ;Debug
	...
	If $debugsetlogTrain = 1 Then Setlog("OCR $sArmyInfo = " & $sArmyInfo, $COLOR_DEBUG) ;Debug
		...
		If IsArray($aGetArmySize) Then
			If $aGetArmySize[0] > 1 Then ; check if the OCR was valid and returned both values
				If Number($aGetArmySize[2]) < 10 Or Mod(Number($aGetArmySize[2]), 5) <> 0 Then ; check to see if camp size is multiple of 5, or try to read again
					If $debugsetlogTrain = 1 Then Setlog(" OCR value is not valid camp size", $COLOR_DEBUG) ;Debug
					ContinueLoop
				EndIf
				$tmpCurCamp = Number($aGetArmySize[1])
				If $debugsetlogTrain = 1 Then Setlog("$tmpCurCamp = " & $tmpCurCamp, $COLOR_DEBUG) ;Debug
				$tmpTotalCamp = Number($aGetArmySize[2])
				If $debugSetlogTrain = 1 Then Setlog("$TotalCamp = " & $TotalCamp & ", Camp OCR = " & $tmpTotalCamp, $COLOR_DEBUG) ;Debug
	...
	If $iTried <= 99 Then
		...
		If $debugsetlogTrain = 1 Then Setlog("$CurCamp = " & $CurCamp & ", $TotalCamp = " & $TotalCamp, $COLOR_DEBUG) ;Debug
	Else
